...
CT::T_USE_TRAIT,
...(\defined('T_ATTRIBUTE') ? [T_ATTRIBUTE] : []), 
T_CATCH,
T_DOUBLE_COLON,
T_DOC_COMMENT,
T_EXTENDS,
...
} elseif (\defined('T_ATTRIBUTE') && $tokens[$index]->isGivenKind(T_ATTRIBUTE)) { 
$this->fixAttribute($tokens, $index, $uses, $namespaceName);
} elseif ($discoverSymbolsPhase && !\defined('T_ATTRIBUTE') && $tokens[$index]->isComment() && Preg::match('/#\[\s*('.self::REGEX_CLASS.')/', $tokens[$index]->getContent(), $matches)) { 
